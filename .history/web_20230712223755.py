...
selected_tasks = st.multiselect('Select tasks:', todos, format_func=lambda x: x.strip())

for index, task in enumerate(todos, start=1):
    checked = task.strip() in selected_tasks
    selected = st.checkbox(f'{index}. {task.strip()}', value=checked, key=f'checkbox-{index}')
    if selected and task.strip() not in selected_tasks:
        selected_tasks.append(task.strip())
    elif not selected and task.strip() in selected_tasks:
        selected_tasks.remove(task.strip())
