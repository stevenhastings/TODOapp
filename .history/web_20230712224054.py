...
for index, task in enumerate(todos, start=1):
    checkbox_state = st.checkbox(f'{index}. {task.strip()}', key=f'checkbox-{index}')
    checkbox_states.append(checkbox_state)
    if checkbox_state:
        selected_tasks.append(task.strip())

# Get selected task indexes
selected_indexes = [index for index, state in enumerate(checkbox_states, start=1) if state]
...
if st.session_state.edit_mode:
    ...
    if selected_index:
        selected_task = todos[selected_index - 1].strip()
        if selected_task in selected_tasks:
            updated_task = st.text_input('Enter the updated task:', value=selected_task, key=f"edit-{selected_index}")
            if st.button('Update', key='update_button'):
                try:
                    funcs.edit_todo(selected_index - 1, updated_task)
                    st.write(f'Task "{selected_task}" updated to "{updated_task}" successfully!')
                    st.session_state.edit_mode = False
                    st.session_state.selected_index = None
                except Exception as e:
        ...
        if selected_task in selected_tasks:
            ...
            if confirm_remove:
                try:
                    ...
                    if task_removed:
                        st.write(f'Task "{selected_task}" removed successfully!')
                        st.session_state.remove_mode = False
                        st.session_state.selected_index = None
                    else:
