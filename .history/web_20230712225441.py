...
def edit_todo():
    for index, todo in enumerate(todos):
        if st.session_state[todo]:
            updated_todo = st.text_input(f'Update "{todo}"', value=todo)
            funcs.edit_todo(index, updated_todo)
            st.write(f'Task "{todo}" updated to "{updated_todo}" successfully!')
            todos[index] = updated_todo  # Update the todos list
