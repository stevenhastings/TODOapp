...
for index, task in enumerate(todos, start=1):
    checkbox_state = st.checkbox(f'{index}. {task.strip()}')
    if checkbox_state:
...
new_todo = st.text_input('Enter a new task:', key='new_todo')
if st.button('Add Task'):
    if new_todo:
        try:
            funcs.add_todo(new_todo)
            st.write(f'Task "{new_todo}" added successfully!')
        except Exception as e:
