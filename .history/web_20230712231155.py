...
def add_todo():
    ...
    funcs.add_todo(todo)
    st.write(f'Task "{todo.strip()}" added successfully!')

def remove_todo():
    for todo in todos:
        if st.session_state[todo]:
            funcs.remove_todo(todo)
            st.write(f'Task "{todo}" removed successfully!')
...
st.write("This app uses a text file to store your to-do list. The file is located at `data/todos.txt`.")

for todo in todos:
    st.checkbox(todo, key=todo)


st.text_input(label="New Task", placeholder="Enter a new task", key="new_todo")
...
if st.button("Edit Selected Tasks"):
    edit_todo()
updated_todo = st.text_input('Enter the updated task:', value=edit_todo, key=f"edit-{funcs.edit_todo(index=selected_index, updated_todo=updated_todo)}")
st.session_state["updated_todo"] = updated_todo
if st.button('Update'):
    edit_todo()
